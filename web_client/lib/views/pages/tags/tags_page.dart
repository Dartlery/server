...
@Component(
    ...
    styleUrls: const ["package:lib_angular/shared.css", "tags_page.css"],
    templateUrl: 'tags_page.html')
class TagsPage extends APage<ApiService> implements OnDestroy {
  ...
  TagsPage(PageControlService pageControl, ApiService api,
      AuthenticationService auth, Router router)
      : super(api, auth, router, pageControl) {
...
//        await _api.tags.update(model, selectedTag.id, selectedTag.category);
  ...
  Future<Null> recountTags() async {
    pageControl.setIndeterminateProgress();
    await performApiCall(() async {
      await this.api.tags.resetTagInfo();
    });
    await refresh();
  }
  ...
  Future<Null> deleteSelected() async {
    ...
    await performApiCall(() async {
      for (TagWrapper tag in selected) {
        if (tag.hasCategory) {
          await this.api.tags.delete(tag.id, tag.category);
        } else {
          await this.api.tags.deleteWithoutCategory(tag.id);
        }
        tags.remove(tag);
  ...
  Future<Null> refresh() async {
    pageControl.setIndeterminateProgress();
    await performApiCall(() async {
      final List<String> categories = await this.api.tagCategories.getAllIds();
      categories.insert(0, "");
      ...
      if (isNullOrWhitespace(tagQuery)) {
        data = await this.api.tags.getAllTagInfo();
      } else {
        if (!showRedirects)
          data = await this.api.tags.search(tagQuery, redirects: false);
        else
          data = await this.api.tags.search(tagQuery);
      }
