...
void main() {
  ...
  TagInfo initialTag, initialCategoryTag, randomTag;
  IdResponse initialItemId;
  setUp(() async {
    if (server != null) throw new Exception("Server already exists");

    server = await setUpServer();
    initialTag = new TagInfo.withValues(testTagName);
    initialCategoryTag = new TagInfo.withValues(testTagName, testCategoryName);
    randomTag = new TagInfo.withValues(generateUuid());
    await api.tagCategories
        .create(new TagCategory.withValues(testCategoryName));
    request = await createItemRequest(
        tags: <Tag>[initialTag, initialCategoryTag, randomTag]);
  ...
  group("Method tests", () {
    test("getTagInfo()", () async {
      final TagInfo info =
          await api.tags.getTagInfoWithoutCategory(testTagName);
    ...
    test("getAllTagInfo()", () async {
      final PaginatedResponse<TagInfo> info = await api.tags.getAllTagInfo();
      expect(info, isNotNull);
    ...
    test("search()", () async {
      ...
      expect(tags.items[1] == initialTag, isTrue);
    });
    ...
    test("setRedirect()", () async {
      final TagInfo redirectRequest =
          new TagInfo.copy(initialTag, initialCategoryTag);
    ...
    test("delete()", () async {
      final CountResponse response =
          await api.tags.deleteWithoutCategory(randomTag.id);
  ...
  group("Validation tests", () {
    test("Reserved regex chars", () async {
      final TagInfo newTag = new TagInfo.withValues(
          reservedRegexChars.join(), reservedRegexChars.join());
      await api.items.updateTagsForItemId(initialItemId.id, [newTag]);
      final TagInfo info =
          await api.tags.getTagInfo(newTag.id, newTag.category);
  ...
  group("Redirect tests", () {
    ...
    test("Chained redirect", () async {
      final TagInfo redirectRequest =
          new TagInfo.copy(initialCategoryTag, randomTag);
    ...
    test("Multiple redirect", () async {
      ...
      await api.items
          .updateTagsForItemId(initialItemId.id, [initialTag, randomTag]);
    ...
    test("Looping redirect", () async {
      ...
      expect(
          api.tags.setRedirect(redirectRequest), throwsInvalidInputException);
    });
    ...
    test("Self redirect", () async {
      ...
      expect(
          api.tags.setRedirect(redirectRequest), throwsDataValidationException);
    });
    ...
    test("Item tags update", () async {
      ...
      await api.items
          .updateTagsForItemId(initialItemId.id, [initialTag, newTag]);
  ...
  group("Tag count tests", () {
    test("Create item", () async {
      final Tag newTag = new Tag.withValues(generateUuid());
      final CreateItemRequest request = await createItemRequest(
          file: "test2.JPG", tags: [initialCategoryTag, newTag]);
      await api.items.createItem(request);
      ...
      info = await api.tags
          .getTagInfo(initialCategoryTag.id, initialCategoryTag.category);
    ...
    test("Merge items", () async {
      ...
      await api.items
          .mergeItems(initialItemId.id, new IdRequest.withValue(response.id));
    ...
    test("Update item tags", () async {
      ...
      await api.items
          .updateTagsForItemId(initialItemId.id, [initialCategoryTag, newTag]);
    ...
    test("Delete tag", () async {
      ...
      expect(api.tags.getTagInfoWithoutCategory(initialTag.id),
          throwsNotFoundException);

      final TagInfo info = await api.tags
          .getTagInfo(initialCategoryTag.id, initialCategoryTag.category);

      expect(info == initialCategoryTag, isTrue);
      expect(info.count, 1);
    });
    test("Out-of-bounds change", () async {
      final AItemDataSource itemDataSource =
          server.injector.get(AItemDataSource);
      final TagInfo newTag =
          await api.tags.getTagInfo(randomTag.id, randomTag.category);
      await itemDataSource.updateTags(initialItemId.id, [newTag]);

      initialTag.redirect = newTag;
      await api.tags.setRedirect(initialTag);

      await api.tags.resetTagInfo();

      final PaginatedResponse<TagInfo> tags = await api.tags.getAllTagInfo();
      expect(tags, isNotNull);
      expect(tags.totalCount, 2);
      expect(tags.items[0] == initialTag, isTrue);
      expect(tags.items[0].count, 0);
      expect(tags.items[1] == newTag, isTrue);
      expect(tags.items[1].count, 1);
    });
  });
}
